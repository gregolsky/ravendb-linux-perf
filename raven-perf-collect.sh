...
#     sudo bash -s -- --service ravendb --duration 20 --nc renderer:9000
#
#   # Docker container, send to S3:
#   curl -fsSL https://gist.github.com/gregolsky/raven_perf/raw | \
#     sudo -E S3_BUCKET=s3://debug-greg/perf-artifacts bash -s -- --docker ravendb
#
#   # Explicit PID (POC / dev):
#   sudo bash raven-perf-collect.sh --pid 12345 --nc renderer:9000
#
# ─── Flags ──────────────────────────────────────────────────────────────────
#
#   Target (pick one):
...
do_capture() {
  info "Recording for ${DURATION}s at ${FREQ}Hz targeting PID $HOST_PID ..."
  if [[ "$CAPTURE_MODE" == "fp" ]]; then
    perf record \
      -F "$FREQ" \
      -g \
...
# ─── 6. Bundle ──────────────────────────────────────────────────────────────
BUNDLE_NAME="raven-perf-$(hostname -s)-$(date +%Y%m%dT%H%M%SZ)"
...
run_demo() {
  ...
  if [[ ! -f "$SCRIPT_DIR/RavenDB/Server/Raven.Server" ]]; then
    ...
    bash "$SCRIPT_DIR/10-get-ravendb.sh"
  fi
  ...
  bash "$SCRIPT_DIR/30-load.sh" --duration "$(( DURATION + 10 ))" &
  LOAD_PID=$!
...
echo ""
echo "═══════════════════════════════════════════════════════"
echo "  RavenDB perf collector  [mode: $MODE_TARGET / $CAPTURE_MODE]"
echo "═══════════════════════════════════════════════════════"
echo ""
